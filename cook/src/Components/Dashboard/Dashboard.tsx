...
import { GetArticleByUserId } from '../../Services/DataService';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const lastViewedRecipesString = localStorage.getItem('lastViewedRecipes') || '[]';
    const lastTwoRecipes = JSON.parse(lastViewedRecipesString).splice(1, 2);

    setLastViewedArticles(lastTwoArticles);
    setLastViewedRecipes(lastTwoRecipes);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (user !== null) {
        let parsedUser = JSON.parse(user); // Parse the 'user' string into an object
       
    
        const recipes = await GetArticleByUserId(parsedUser.id);
        setUserRecipes(recipes);
      }
    
      let articleRes = await GetAllArticel();
      setArticelItems(articleRes);
    };
          ...
          <div className="container h-65 mx-auto md:border-spacing-y-96 bg-slate-50 mt-16 rounded-md border md:border-black lg:border-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 md:gap-20 p-5 gap-y-6 md:p-14">
              <div className="col1 rounded-md p-4 col-span-1">
                ...
                <div className="flex relative flex-col items-center mt-4 lg:text-base md:text-xs">
                <h4 className='text-xl font-semibold'>Recipes</h4>
                {lastViewedRecipes.map((item: { id: string, image: string, title: string, description: string }, index: number) => (
                    <React.Fragment key={index}>
                      <Link to={"/RecipeDisplay"} state={{ num: parseInt(item.id) }}>
                 ...
                 <p className='text-xl font-semibold pt-2'>Articles</p>
              ...
              <div className="w-full col-span-2">
                <div className="flex flex-col lg:text-base md:text-xs md:flex-row gap-4">
                  <div className="flex-1 p-4 rounded-md bg1">
                    ...
                    {userRecipes.map((item: {id:number, image: string, title: string, description: string }, index: number) => (
                      <Link to={"/RecipeDisplay"} state={{ num: item.id }}>
                      <img src={item.image} alt="" className="object-cover w-full h-28  p-2  rounded-xl mt-2" />
                      <h5 className='font-serif lg:text-lg text-center ' >{item.title}</h5>
                      <p className="font-serif mr-6 md:ml-3 text-s p-1">{item.description.slice(0, 60)}</p>
                    </Link>
                    ))}
                    <Link to="/Search">
